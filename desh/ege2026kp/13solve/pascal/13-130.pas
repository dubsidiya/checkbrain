...
var k:=0;
    foreach var y in s do
      ...
      if ((y and 216)=216) and (z and 35=0) and not(addr(255,255,y,z).Contains('0.1')) then 
       k:=max(addr(255,255,y,z).Cnt('0'), k); 
        print(2**k);
...
foreach var z in q do
 begin    
  var x:= new CalcIP('156.133.216.35', '255.255.'+z+'.0');
  var n:=x.GenAddr.sel(t -> t.ToS).Sel(t -> t[ :t.IndexOf(',')+1]).toA;  
  if (n.Cnt>0) and (n[0]='156.133.216.0') then m:=max(m, x.GenAddr.cnt);  
  end;  
  print(m+2)                