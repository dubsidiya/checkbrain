...
var s: HashSet<integer>:=[0, 128, 192, 224, 240, 248, 252, 254, 255];
  foreach var x in s do 
    foreach var y in s do
      ...
      if (net(x and 128)=net(128))and ((y and 112)=64) and (z and 142=0) then 
      println(addr(255,x,y,z),y); 
      
//другое решение 
var q: HashSet<string>:=['0', '128', '192', '224', '240', '248', '252', '254'];
var set1:='124.128.64.0'; 
foreach var z in q do
 begin    
  var f:= new CalcIP('124.128.112.142', '255.255.'+z+'.0');
  var n:=f.GenAddr.sel(t -> t.ToS).Sel(t -> t[ :t.IndexOf(',')+1]).toA;  
