...
for s in open("27-89b.txt"):
    x, y = map(float,s.replace(',','.').split())
    if y > 3/2*x-7.5 and y>-3/4*x+12.75: klas[0].append( (x, y) )
    elif y > 3/2*x-7.5 and y<-3/4*x+12.75: klas[1].append( (x, y) )
    elif y < 3/2*x-7.5 and y<-3/4*x+12.75: klas[2].append( (x, y) )
    else: klas[3].append( (x, y) )
cent = []    # центроиды
